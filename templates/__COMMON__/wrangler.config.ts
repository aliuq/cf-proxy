...
async function wranglerConfig(options: WranglerConfigOptions<INTERNAL_ENV>): Promise<WranglerConfig> {
  ...
  return {
    ...
    env: {
      ...
      localhost: {
        vars,
        route: { pattern: `foo.${domain.local}`, zone_name: domain.local, custom_domain: true },
      },
      preview: {
        vars,
        route: domain.prod
          ? { pattern: `foo-preview.${domain.prod}`, zone_name: domain.prod, custom_domain: true }
          : undefined,
      },
      production: {
        vars,
        route: domain.prod
          ? { pattern: `bar.${domain.prod}`, zone_name: domain.prod, custom_domain: true }
          : undefined,
      },
    },
